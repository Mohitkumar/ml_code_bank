...
data['Description'] = data['Description'].apply(lambda x: [item for item in x if item not in stop]).apply(lambda x: ' '.join(x))
...
model.fit(X_train, y_train, nb_epoch=15, batch_size=64)
